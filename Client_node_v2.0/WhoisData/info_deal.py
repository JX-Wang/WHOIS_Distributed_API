...
def get_result(domain_punycode, tld, whois_addr, func_name, data, flag, proxy_msg):
    """
        ...
        try:
            ...
            if whois_server_sec:  # 如果获取到了二级whois地址,更新sec_whois并重新获取数据
                ...
                whois_details_sec = GetWhoisInfo(domain_punycode,
                                                 whois_server_sec, proxy_msg).get()
...
# 用来判断com_manage函数中，得到的whois信息是否包含xxx标志，若包括则有二级whois服务器，需要重新探测
def xxx_bool(data):
