...
from SQL_generator.sql_generator import SQL
...
def get_domain_whois(raw_domain, whois_server):
    """
    获取whois信息
    ...
    support_json_path = os.path.abspath('./')
    support_json = file(support_json_path + '/support.json')
    support = json.load(support_json)  # 打开support文件

    if whois_server is '':

        # 以下是经过数据库的whois服务器以及对应的ip地址的获取方法 #####################################################
        ...
        # print WhoisSerAddr, WhoisSerAddr, WhoisFunc
        # ###################################################################################################

        # 以下是经过本地support文件堆whois服务器对应的ip地址获取方法 #################################################

        WhoisSerAddr = support['TLD_WHOISSERVER'][tld]  # 获取whois服务器地址
        if WhoisSerAddr.find(','):
            WhoisSerAddr = WhoisSerAddr[WhoisSerAddr.find(',') + 1:]  # 使用第二个whois服务器
        else:
            WhoisSerAddr = WhoisSerAddr
        WhoisSerIP = support['WHOISSERVER_IP'][WhoisSerAddr]  # 获取Whois服务器IP地址
        if WhoisSerIP.find(','):
            WhoisSerIP = WhoisSerIP[WhoisSerIP.find(',') + 1:]  # 获取whois服务器对应ip，本程序采用whois服务器探测
        else:
            WhoisSerIP = WhoisSerIP
        WhoisFunc = Resource.WhoisFunc.get_whois_func(WhoisSerAddr)  # 获取TLD对应的提取函数名称
        # ###################################################################################################
        ...
        # 获取用于通信的whois服务器地址
        # 优先级 : whois地址 > None (失败) 取消使用ip进行与whois服务器之间的通信
        ...
        try:
            raw_whois_data = whois_connect.GetWhoisInfo(domain_punycode, WhoisConnectAddr).get()  # 未通过socks的whois请求
        ...
        if whois_dict and whois_dict.has_key('flag'):  # 获取了正确的WHOIS数据
            try:
                DB.execute(SQL_generator.insert_whois(whois_dict))
                DB.commit()
                log_get_whois.info("domain", domain_punycode, "commit")
    ...
    else:  # 指定whois服务器的whois探测
        WhoisSerAddr = whois_server
        print '指定whois服务器查询 -> domain -> ', raw_domain, 'whois_server :', WhoisSerAddr
        WhoisFunc = Resource.WhoisFunc.get_whois_func(WhoisSerAddr)  # 获取TLD对应的提取函数名称
        # ###################################################################################################
...
def whois_query(raw_domain, whois_server):
    """API核心函数 获取域名的WHOIS并记录日志"""
    start = time.time()
    result = get_domain_whois(raw_domain, whois_server)  # whois 获取
    end = time.time()
    if result:
