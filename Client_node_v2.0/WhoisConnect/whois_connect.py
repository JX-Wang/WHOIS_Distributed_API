...
# 错误列表
error_list = ["ERROR -1",   # timeout / TTL expired
              "ERROR -2",   # whois服务器不可达etc
              "ERROR -3",   # 接收数据出现错误
              "ERROR OTHER"]  # others
...
class GetWhoisInfo:
    ...
    def __init__(self, domain, whois_srv, proxy_msg):
        """处理whois服务器"""
        # 处理特殊的请求格式
    ...
    def get(self):
        """获取数据"""
        data = ''
        for i in range(RECONNECT):  # 最大重连数,修改后系统只与whois服务器建立一次连接
            data = self._connect()
    ...
    def _connect(self):
        ...
        # whois服务器ip，代理ip
        # global _proxy_socks
        host = self.whois_srv  # 如果ip地址为空则使用服务器地址
        self.tcpCliSock = socks.socksocket()  # 创建socket对象
        self.tcpCliSock.settimeout(TIMEOUT)  # 设置超时时间
        if self.Proxy_Flag:  # socks代理配置
            if self.proxy_info is not None:
                # 设置代理
                if self.proxy_info['mode'] == 'SOCKS5':
                    for key in self.proxy_info:
                        print key, ':', self.proxy_info[key]
                    self.tcpCliSock.set_proxy(proxy_type=socks.SOCKS5,  # socks类型
                                              addr=self.proxy_info['ip'],  # socks代地址
                                              port=self.proxy_info['port'],  # 端口
                                              username=self.proxy_info['username'],  # 用户名
                                              password=self.proxy_info['password'])  # 密码
                elif self.proxy_info['mode'] == 'SOCKS4':
